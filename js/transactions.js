...
export class TransactionVisualizer {
    constructor(earth) {
        ...
        this.whaleFilterActive = false;
        this.searchHighlightAddress = null;
    }

    setWhaleFilter(active) {
        this.whaleFilterActive = active;
        // Optionally clean up existing non-whale beams
        if (active) {
            this.transactionBeams.forEach(beam => {
                if (beam.userData && beam.userData.isTransactionBeam && !beam.userData.isWhale) {
                    beam.visible = false;
                }
            });
        } else {
            this.transactionBeams.forEach(beam => beam.visible = true);
        }
    }

    setSearchHighlight(address) {
        this.searchHighlightAddress = address ? address.trim() : null;
        if (this.searchHighlightAddress) {
            console.log("Søger efter adresse:", this.searchHighlightAddress);
        }
    }
    ...
    createTransactionBeam(fromAddress, toAddress, amount, txHash) {
        ...
        // Mulighed: Rute gennem en tilfældig validator for "smoothness" og netværks følelse
        const useValidatorRouting = Math.random() < 0.2;
        let midPoint;
        ...
        // Beregn midPoint for Bezier kurve
        let midPos;
        if (useValidatorRouting) {
            // Find en tilfældig validator marker
            const markers = Array.from(window.xrpApp.validatorNetwork.validatorMarkers.values());
            if (markers.length > 0) {
                const randomMarker = markers[Math.floor(Math.random() * markers.length)];
                midPos = randomMarker.position.clone().multiplyScalar(1.05); // Lidt over overfladen
            } else {
        ...
        const points = curve.getPoints(64); // Mere smooth (64 points i stedet for 50)
        ...
        if (xrpAmount > 1000) { // Over 1k XRP
            color = 0xffaa00; // Orange for store beløb
        }
        if (xrpAmount > 10000) { // Over 10k XRP
            color = 0xff0088; // Pink for whale beløb
            isWhale = true;
        }

        // Tjek search highlight
        const isHighlighted = (this.searchHighlightAddress && (fromAddress === this.searchHighlightAddress || toAddress === this.searchHighlightAddress));
        if (isHighlighted) {
            color = 0xffffff; // Hvid for søgte adresser
        }

        // Whale filter
        if (this.whaleFilterActive && !isWhale && !isHighlighted) {
            return null;
        }
        ...
        beam.userData = {
            ...
            isHighlighted,
            startTime: Date.now(),
            duration: isHighlighted ? 6000 : 3000, // Længere duration for highlights
            isTransactionBeam: true
        };

        if (isHighlighted) {
            material.opacity = 0.8;
            material.linewidth = 2; // Virker kun i nogle browsere/renderers
        }
