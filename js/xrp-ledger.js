...
export class XRPLedgerManager {
    ...
    addNewBlock(ledgerData) {
        ...
        // Beregn Ledger Close Time (LCT)
        const now = Date.now();
        if (this.lastLedgerTime) {
            const lct = (now - this.lastLedgerTime) / 1000;
            this.avgLct = this.avgLct === 0 ? lct : this.avgLct * 0.7 + lct * 0.3; // Glidende gennemsnit
        }
        this.lastLedgerTime = now;

        // Beregn TPS (over de sidste 5 blokke)
        this.tpsHistory.push({
            count: ledgerData.txn_count || 0,
            time: now
        });
        if (this.tpsHistory.length > 5) this.tpsHistory.shift();

        if (this.tpsHistory.length > 1) {
            const totalTxns = this.tpsHistory.reduce((sum, item) => sum + item.count, 0);
            const timeSpan = (this.tpsHistory[this.tpsHistory.length - 1].time - this.tpsHistory[0].time) / 1000;
            this.currentTps = timeSpan > 0 ? totalTxns / timeSpan : 0;
        }
    ...
    updateNavbarStats() {
        const tpsEl = document.getElementById('nav-tps');
        const lctEl = document.getElementById('nav-lct');

        if (tpsEl) tpsEl.textContent = this.currentTps.toFixed(1);
        if (lctEl) lctEl.textContent = this.avgLct.toFixed(1) + ' s';
    }
